...
#define CHARACTERISTIC_UUID "00000002-0000-1000-8000-00805F9B34FB"
#define ENV_DATA_CHARACTERISTIC_UUID "00000003-0000-1000-8000-00805F9B34FB"

#define TOUCH_SENSOR_PIN D2
#define BUZZER_PIN D3
#define VIBRATION_MOTOR_PIN D4

BLEService gestureService(SERVICE_UUID);
BLECharacteristic gestureCharacteristic(CHARACTERISTIC_UUID, BLEWriteWithoutResponse | BLENotify, sizeof(int));
BLECharacteristic envDataCharacteristic(ENV_DATA_CHARACTERISTIC_UUID, BLERead | BLENotify, 2 * sizeof(float));
...
bool inLSM9DS1Mode = false;

unsigned long lastEnvDataSent = 0;
const unsigned long ENV_DATA_INTERVAL = 5000;
...
namespace {
  ...
  void SetupIMU() {
    ...
    gyroscope_sample_rate = IMU.gyroscopeSampleRate();
      ...
      MicroPrintf("Acceleration sample rate %d.%d Hz",static_cast<int32_t>(rate_int),static_cast<int32_t>(rate_frac * 100));
      rate_frac = modf(gyroscope_sample_rate, &rate_int);
      MicroPrintf("Gyroscope sample rate %d.%d Hz", static_cast<int32_t>(rate_int),static_cast<int32_t>(rate_frac * 100));
...
void setup() {
  ...
  Serial.println("Testing buzzer and vibration motor...");
  tone(BUZZER_PIN, 1000, 1000);
  delay(1000);
  digitalWrite(VIBRATION_MOTOR_PIN, HIGH);
  delay(1000);
  digitalWrite(VIBRATION_MOTOR_PIN, LOW);
  Serial.println("Buzzer and vibration motor test complete.");
  ...
  gestureService.addCharacteristic(envDataCharacteristic);
...
void loop() {
  ...
  if (digitalRead(TOUCH_SENSOR_PIN) == HIGH) {
    if (!touchDetected) {
      touchDetected = true;
      inLSM9DS1Mode = true;
      Serial.println("Touch detected. Entering gesture mode.");
    }
    detectLSM9DS1Gesture();
  } else {
    if (touchDetected) {
      touchDetected = false;
      inLSM9DS1Mode = false;
      Serial.println("Touch released. Exiting gesture mode.");
    }
    detectAPDS9960Gesture();
  }

  // Send environmental data periodically
  if (currentMillis - lastEnvDataSent >= ENV_DATA_INTERVAL) {
    sendEnvironmentalData();
    lastEnvDataSent = currentMillis;
  }
...
void sendEnvironmentalData() {
  ...
  // Prepare data array
  float envData[2] = {temperature, humidity};

  // Send data via BLE
  envDataCharacteristic.writeValue(reinterpret_cast<uint8_t*>(envData), sizeof(envData));

  Serial.println("Environmental data sent via BLE");
  Serial.println();
  delay(1000);
}

void humidityFunction() {
  Serial.println("Humidity function called");
  sendEnvironmentalData();
}
  ...
  if (APDS.gestureAvailable()) {
    ...
    switch (gesture) {
      case GESTURE_UP:
        gestureStr = "UP";
        max_index = 11;
        gestureCharacteristic.writeValue(reinterpret_cast<const uint8_t*>(&max_index), sizeof(max_index));
        break;
      case GESTURE_DOWN:
        gestureStr = "DOWN";
        max_index = 12;
      ...
      case GESTURE_RIGHT:
        ...
        break;
      default:
        return; // Ignore unrecognized gestures
    }

    Serial.println("Detected " + gestureStr + " gesture");
    // gestureCharacteristic.writeValue(gestureStr.c_str(), gestureStr.length());
  }
}

void detectLSM9DS1Gesture() {
  if (!touchDetected) {
    return;
  }
  ...
  if (done_just_triggered) {
    ...
    if (max_score_int <= 80) {
      MicroPrintf("Failed to recognize gesture");
      ...
      digitalWrite(VIBRATION_MOTOR_PIN, HIGH);
      delay(1000);  // Vibrate for 200 ms
      digitalWrite(VIBRATION_MOTOR_PIN, LOW);
      Serial.println("Vibration motor activation complete.");
